...
export default function Team() {
  ...
  useEffect(() => {
    ...
    if (savedDepartments) {
      ...
      setExpandedDepartments(new Set(depts.map((d: Department) => d.id).concat(['uncategorized'])));
    }
  ...
  const handleTeamMemberSave = (userData: Omit<User, "id">) => {
    if (editingUser) {
      // Update existing user
      const updatedTeamMembers = teamMembers.map(member =>
        member.id === editingUser.id ? { ...userData, id: editingUser.id } : member
      );
      setTeamMembers(updatedTeamMembers);
      localStorage.setItem("taskflow_team_members", JSON.stringify(updatedTeamMembers));
      toast({ 
  ...
  const handleDepartmentSave = (name: string) => {
    ...
    localStorage.setItem("taskflow_departments", JSON.stringify(updatedDepartments));
              ...
              <div className="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity group">
                ...
                <ChevronDown 
                  className={`h-5 w-5 transition-transform ${
                    expandedDepartments.has(departmentGroup.id) ? "rotate-0" : "-rotate-90"
                  }`}
                />
              </div>
            </CollapsibleTrigger>
